...
function stopScroll() { 
    ...
    setTimeout(() => {
        ...
        if (contextOptions) {
            ...
            // Seleciona aleatoriamente a quantidade de diamantes (de 1 a 7)
            const diamondCount = Math.floor(Math.random() * 7) + 1;

            // Embaralha os itens da grade
            const shuffledItems = Array.from(gridItems).sort(() => Math.random() - 0.5);

            // Adiciona os diamantes aos itens selecionados aleatoriamente
            for (let i = 0; i < diamondCount; i++) {
                const selectedItem = shuffledItems[i]; // Seleciona o item do embaralhamento
                if (selectedItem) {
